...
public class CommandDispatcher<ModelType> extends AbstractApiRequestDispatcher<ModelType, ProcessUserCommandResponse,
    ...
    protected ProcessUserCommandResponse serializeErrorObject(ApiServerException exception) {
        ...
        if (exception instanceof UnknownRequestException) {
            ...
            return new ProcessUserCommandResponse("Error: did not recognise command '" + name + "'. " +
                    "You may be trying to use a command that is only available in a higher-level channel " +
                    "(i.e. `main` or `admin`).", true);
        } else if (ClassUtils.instanceOfAny(exception, allowedExceptionsForUserMessage)) {
            return new ProcessUserCommandResponse("Error: " + exception.getMessage(), true);
        } else {
            return new ProcessUserCommandResponse("Sorry, an unknown error occurred.", true);
        }
    }
