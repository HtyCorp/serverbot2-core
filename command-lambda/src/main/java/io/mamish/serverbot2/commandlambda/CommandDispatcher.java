...
public class CommandDispatcher extends AbstractApiRequestDispatcher<ICommandHandler,CommandServiceResponse,
        CommandServiceRequest,CommandServiceRequest> {
    ...
    @Override
    protected CommandServiceResponse serializeErrorObject(ApiServerException exception) {
        // UnknownRequestException message is generated by the superclass, but still needs a user-friendly message.
        if (exception instanceof UnknownRequestException) {
            String name = ((UnknownRequestException)exception).getRequestedTargetName();
            return new CommandServiceResponse("Error: '" + name + "' is not a recognised command.");
